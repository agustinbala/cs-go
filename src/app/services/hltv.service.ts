  ...
  async getMatchDetail(link: string) {
    ...
    try {
      ...
      allMaps.map((_i, element) => {
        const el = $(element);
        const map = el
          .find(".map-stats-infobox-mapname-container")
        ...
        const team1MapDetailLink = team1El
          .find(".map-stats-infobox-winpercentage")
          .children("a")
          .attr("href")!;
        const team2MapDetailLink = team2El
          .find(".map-stats-infobox-winpercentage")
          .children("a")
          .attr("href")!;
        const team1WinPercentage = team1El
          .find(".map-stats-infobox-winpercentage")
          .children("a")
          .text();
        const team2WinPercentage = team2El
          .find(".map-stats-infobox-winpercentage")
          .children("a")
          .text();
        const team1MapTimesPlayed = team1El
          .find(".map-stats-infobox-maps-played")
          .text();
        const team2MapTimesPlayed = team2El
          .find(".map-stats-infobox-maps-played")
          .text();
        const team1Pick = team1El.find(".map-stats-infobox-pick").length != 0;
        const team2Pick = team2El.find(".map-stats-infobox-pick").length != 0;
        const banTeam1 = team1El.find(".map-stats-infobox-ban").length != 0;
        const banTeam2 = team2El.find(".map-stats-infobox-ban").length != 0;
        ...
        const mapStats: HLTVMapStats = {
          ...
          team1Pick,
          team2Pick,
          isPick: team1Pick || team2Pick || (!banTeam1 && !banTeam2),
        };
